...
PhyjixWorld::~PhyjixWorld()
{
    ...
    if (_controllerManager) _controllerManager->release();
}
...
void PhyjixWorld::RemoveRigidBody(IRigidBody* body)
{
    ...
    _scene->removeActor(*physxBody->GetActor());
}

ICharacterController* PhyjixWorld::CreateCharacterController(const DirectX::SimpleMath::Vector3& position, float radius,
	float height)
{
...
{
    auto it = std::find_if(_characterControllers.begin(), _characterControllers.end(),
        [controller](const ICharacterController* ptr) { return ptr == controller; });
...
void PhyjixWorld::CreateDefaultGround()
{
    ...
    groundPlane = PxCreatePlane(*_physics, physx::PxPlane(0, 1, 0, 50), *mMaterial);
